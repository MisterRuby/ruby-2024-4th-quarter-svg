## svg 태그 내에 style / script 코드 포함

- svg 태그 내에서도 style 및 script 태그를 작성할 수 있다.
  - svg 태그 내의 요소들에 대해서만 적용할 style 및 script 를 작성할 때 구분하기 위한 용도로 유용함

```html
<svg class="face" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 571 625.5">
  <style>
    .face {
      position: absolute;
      top: 0;
      bottom: 0;
      left: 0;
      right: 0;
      width: 200px;
      margin: auto;
    }

    @keyframes eye-ani {
      0% {
        transform: scale(0.9);
      }

      100% {
        transform: scale(1.1);
      }
    }
    .face_eye {
      /* transform-origin: center center; */
      /* animation: eye-ani 1s infinite alternate; */
    }
    .ani_on .face_eye {
      animation: eye-ani 1s infinite alternate;
    }

    .face_eye:nth-of-type(2) {
      transform-origin: 175px 380.5px;
    }
    .face_eye:nth-of-type(3) {
      transform-origin: 397px 380.5px;
    }

    .face_hair {
      fill: brown; /* svg 태그에서는 배경색 변경시 background 가 아닌 fill 속성을 사용한다. */
    }
    .face_hair:nth-of-type(1) {
      fill: blue;
    }
  </style>
  <script>
    const faceElem = document.querySelector(".face");
    faceElem.addEventListener("click", function () {
    this.classList.toggle("ani_on"); });
  </script>
  <circle
    cx="286.5"
    cy="354.5"
    r="251"
    stroke-width="40"
    fill="none"
    stroke="#000"
    stroke-miterlimit="10" />
  <circle class="face_eye" cx="175" cy="380.5" r="42.5" />
  <circle class="face_eye" cx="397" cy="380.5" r="42.5" />
  <path
    d="M422 476.16a181.3 181.3 0 0 1-135.48 60.59H286a181.3 181.3 0 0 1-135.48-60.59"
    stroke-width="30"
    fill="none"
    stroke="#000"
    stroke-miterlimit="10" />
  <ellipse class="face_hair" cx="168.5" cy="210.5" rx="168.5" ry="121.5" />
  <ellipse class="face_hair" cx="428.5" cy="198.5" rx="142.5" ry="123.5" />
  <ellipse class="face_hair" cx="245.5" cy="103" rx="151.5" ry="103" />
</svg>
```
